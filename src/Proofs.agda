...
compTeIgnSh : {scΓ : S.Context sΓ} → 
    ...
    (i : ℕ) → 
    (p : i ≤ conLen sΓ) → 
    compileTerm (insertSkip rΓ i p sB) (S.shiftindices sa 1 i) ≡ compileTerm rΓ sa
compTeIgnSh (S.var x) rΓ i p = {!   !}
compTeIgnSh (S.ƛ∶ sA 𝕢 𝟘 ♭ sa) rΓ i p = compTeIgnSh sa (rΓ ,ᵣ sA skip) (suc i) (s≤s p)
compTeIgnSh (S.ƛ∶ sA 𝕢 ω ♭ sa) rΓ i p 
    rewrite compTyIgnShift {i = 1} {l = i} sA | compTeIgnSh sa (rΓ ,ᵣ sA ↦ _) (suc i) (s≤s p) = {!   !}
compTeIgnSh (S.ƛr∶ x ♭ sa) rΓ i p = refl
compTeIgnSh (sa S.· sa₁ 𝕢 𝟘) rΓ i p = compTeIgnSh sa rΓ i p
compTeIgnSh (sa S.· sa₁ 𝕢 ω) rΓ i p = {!   !}
compTeIgnSh (sa S.·ᵣ sa₁) rΓ i p = compTeIgnSh sa₁ rΓ i p
compTeIgnSh S.z rΓ i p = refl
compTeIgnSh {sB = sB} (S.s sa) rΓ i p rewrite compTeIgnSh {sB = sB} sa rΓ i p = refl
compTeIgnSh S.nill rΓ i p = refl
compTeIgnSh (sa S.∷l sas) rΓ i p = {!   !}
compTeIgnSh (S.nilv𝕢 𝟘) rΓ i p = refl 
compTeIgnSh (S.nilv𝕢 ω) rΓ i p = refl 
compTeIgnSh (sa S.∷v sas 𝕟 sn 𝕢 𝟘) rΓ i p = {!   !}
compTeIgnSh (sa S.∷v sas 𝕟 sn 𝕢 ω) rΓ i p = {!   !}
compTeIgnSh (S.elimnat sa P∶ sa₁ zb∶ sa₂ sb∶ sa₃) rΓ i p = {!   !}
compTeIgnSh (S.eliml sa ty∶ innerty P∶ sa₁ nb∶ sa₂ cb∶ sa₃) rΓ i p = {!   !}
compTeIgnSh (S.elimv x ty∶ innerty P∶ sa nb∶ sa₁ cb∶ sa₂) rΓ i p = {!   !}
compTeIgnSh S.Nat rΓ i p = refl 
compTeIgnSh (S.List x) rΓ i p = refl 
compTeIgnSh (S.Vec sa (n 𝕢 σ)) rΓ i p = refl 
compTeIgnSh (S.∶ A 𝕢 σ ⟶ x₁) rΓ i p = refl 
compTeIgnSh (S.r∶ x ⟶ x₁) rΓ i p = refl 
compTeIgnSh (S.Sett x) rΓ i p = refl
